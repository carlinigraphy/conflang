...
.Required
----
# Required declaration with no type.
age_1;

# Required declaration w/ type.
age_2 (int);
----
...
If a variable is declared in the parent with a type, the type may not be over-written in the child.
Expressions defined in the parent may be over-written in the child.
...
.child.conf
----
# Type Error: cannot overwrite type from ./parent.conf
name (path): './path';
----
