...
If the `p_statement` is an `%include`, it doesn't create a `$NODE`.
Ooh, an interseting idea... maybe it *should* create a node.
Make a sentinel NODE whose type is '`include`', and the index for which is saved in the actual `INCLUDE` node along with the `.target`.
Doesn't need to be a real NODE_* with a `TYPEOF{}`, probably just something called `REPLACE_ME`.
